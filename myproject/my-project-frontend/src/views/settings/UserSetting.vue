...
const registerTime = computed(() => new Date(store.user.registerTime).toLocaleString('zh',{hour12: false}).replaceAll('/','-'))
...
<template>
  <div style="display: flex;max-width: 950px;margin: auto">
    <div class="settings-left">
      <card :icon="User" title="账号信息设置" desc="在这里编辑个人信息，您可以在隐私设置中选择是否展示这些信息" v-loading="loading.form">
      ...
      <card style="margin-top: 10px" :icon="Message" title="电子邮件设置" desc="您可以在这里修改默认的电子邮件地址">
        <el-form :model="emailForm" @validate="onValidate" :rules="rules" ref="emailFormRef" label-position="top" style="margin: 0 10px 10px 10px">
          ...
          <el-form-item prop="code">
            <el-row style="width: 100%" :gutter="10">
              ...
              <el-col :span="6">
                <el-button  type="success" style="width: 100%" :disabled="!isEmailValid || coldTime > 0" @click="sendEmailCode" plain>{{ coldTime > 0 ? `请稍后 ${coldTime} 秒` : '获取验证码' }}</el-button>
              </el-col>
            </el-row>
          </el-form-item>
          <div>
