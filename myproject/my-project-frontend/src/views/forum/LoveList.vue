...
watch(() => loves.type, () => resetList(), {immediate: true})
...
function updateList(){
  if(loves.end) return
  get(`/api/forum/list-love?page=${loves.page}&type=${loves.type}`, data => {
    if(data) {
      data.forEach(d => loves.list.push(d))
      loves.page++
    }
  ...
  <div style="display: flex;margin: 20px auto;gap: 20px;max-width: 900px">
    <div style="flex: 1">
      ...
      <transition name="el-fade-in" mode="out-in">
        <div v-if="loves.list.length">
          <div style="margin-top: 10px;display: flex;flex-direction: column;gap: 10px"
               v-infinite-scroll="updateList">
            <light-card style="height: 150px" v-for="item in loves.list" class="topic-card" @click="router.push('/index/love-detail/'+item.id)">
    ...
    <love-editor :show="editor" @success="onLostCreate" @close="editor = false"/>
